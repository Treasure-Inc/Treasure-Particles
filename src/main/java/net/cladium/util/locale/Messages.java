package net.cladium.util.locale;
...
public class Messages {
    ...
    public Messages() {
        File file = new File(CladiumPlugin.getInstance().getDataFolder(), "messages.yml");
        if (!file.exists()) {
            CladiumPlugin.getInstance().saveResource("messages.yml", false);
        }
    }

    public void load() {
        try {
            YamlConfiguration config = YamlConfiguration.loadConfiguration(new File(CladiumPlugin.getInstance().getDataFolder(), "messages.yml"));

            RELOADED = ChatColor.translateAlternateColorCodes('&', config.getString("reloaded", "§aReloaded!"));
            EFFECT_SELECTED = ChatColor.translateAlternateColorCodes('&', config.getString("effect-selected", "§bSelected: %s"));
            EFFECT_NO_PERMISSION = ChatColor.translateAlternateColorCodes('&', config.getString("effect-no-permission", "§cYou cannot use that effect!"));
            EFFECT_UNKNOWN = ChatColor.translateAlternateColorCodes('&', config.getString("effect-unknown", "§cThere is no effect named %s"));
            ...
            RELOADING = ChatColor.translateAlternateColorCodes('&', config.getString("reloading", "§eReloading configurations..."));
            RELOADED = ChatColor.translateAlternateColorCodes('&', config.getString("reloaded", "§aReloaded!"));
